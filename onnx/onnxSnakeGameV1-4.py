...
class Snake:
    ...
    def turn(self, point):
        if self.length > 1 and (direction_map[point][0] * -1, direction_map[point][1] * -1) == self.direction:
            return
        else:
            self.direction = direction_map[point]
...
def get_move_from_onnx(snake, food):
    ...
    return direction_index, confidence
...
def game_loop():
    ...
    while not game_over:
        ...
        logging.info(f"Head: ({new_head[0]}, {new_head[1]}), Food: ({food.position[0]}, {food.position[1]}), Move: {actual_move}, Confidence: {confidence:.2%}")
